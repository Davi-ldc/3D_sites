...
//Controls
// a ideia do OrbitControls é que ele vai atualizar a camera automaticamente
const Crontrols = new OrbitControls(camera, canvas)//ele recebe a camera e o canvas 
Crontrols.enableDamping = true
Crontrols.dampingFactor = 0.05 //controla a intencidade do damping padrão = 0.05
// Animate
...
const tick = () =>
{
    ...
    Crontrols.update()
    //então o efeito de Damping faz com que quando eu solte o mause ele ainda continue mechendo divagarinho
    //ate parar. pra ele frz isso precisa ser atualizado a cada frame.
    
    // Update objects
    //mesh.rotation.y = elapsedTime;

    // camera.position.x = Math.sin(cursor.x * Math.PI *2) * 2 // o vezes 2 é pq sin e cos retorna valores entre 0 e 1 ai a camera fica
    // camera.position.z = Math.cos(cursor.x * Math.PI *2) * 2//mt perto do cubo
    //cm eu quero q ele complete uma volta inteira eu to passando pi*2
